...
parser.add_argument('--modelpath', help='saved model path', type=str, default='models/level1')
parser.add_argument('--logpath', help='saved log path', type=str, default='logs/level1')
...
writer = SummaryWriter(args.logpath) ## 创建一个SummaryWriter的示例，默认目录名字为runs
...
if args.level < 1:
    data_transforms_train_list.append(transforms.Scale(crop_size))
if args.level >= 1:
    data_transforms_train_list.append(transforms.RandomSizedCrop(crop_size))
...
data_transforms_train_list.append(transforms.Normalize([0.5,0.5,0.5], [0.5,0.5,0.5]))
...
if __name__ == '__main__':
    ...
    torch.save(model.state_dict(),os.path.join(args.modelpath,'model.pt'))
